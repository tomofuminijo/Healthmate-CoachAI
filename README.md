...
- 運動や食事に関する実践的な指導
- モチベーション維持のためのサポート
- **長期メモリ対応**: 4つのメモリ戦略による継続的な学習と記憶
- **システムプロンプト管理**: 環境別の人格・口調設定
...
- **長期メモリ戦略**: 4つの専門化されたメモリ戦略による高度な記憶機能
  - **Summary Memory**: セッション単位での会話要約
  - **Semantic Memory**: 意味的な情報の長期記憶
  - **User Preference Memory**: ユーザー設定・嗜好の記憶
  - **Episodic Memory**: エピソード記憶と振り返り機能
